...
bool ImGui::OutputConsoleMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return OutputTextEx(label, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
...
bool ImGui::OutputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            const char * error = strstr(buf_display, "[error]");
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            ImU32 col2 = ImGui::GetColorU32(ImVec4(1.0f, 0.4f, 0.4f, 1.0f));
                        //call recursive function which takes in required parameters, plus the previously drawn buffer text
            if(error)
            {
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col2, error, error + strlen(error), 0.0f, is_multiline ? NULL : &clip_rect);
            ...
            {
                if(i%2)
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col2, buf_display+9, buf_display_end+9, 0.0f, is_multiline ? NULL : &clip_rect);
                else 
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            }*/
        }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if(error)
            {
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col2, error, error + strlen(error), 0.0f, is_multiline ? NULL : &clip_rect);
